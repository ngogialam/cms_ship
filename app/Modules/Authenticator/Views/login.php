        ...
        <div class="row">
            <div class="col-lg-4 col-md-6 col-sm-12 col-xs-8 ml-auto login-form">
                ...
                <form action="" id="form-search-user" method="POST" enctype="multipart/form-data">
                    <div class="input-username-login">
                        <input class="form-control frm-lg userPhone" id="username" name="username" placeholder="<?= lang('Label.lbl_username') ?>"
                            type="text" value="<?= $oldData['username'] ?>"
                            oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*?)\..*/g, '$1');">
                        <span class=" err_messages err_phone"><?php echo $getErrors['username']; ?></span>
                    </div>

                    <div class=" ">
                        <input class="form-control frm-lg pass_log_id" id="password" name="password"
                            data-rule="minlen:4" value="<?= $oldData['password'] ?>"
                            data-msg="Please enter at least 4 chars" placeholder="<?= lang('Label.lbl_password') ?>"
                            type="password">
                    ...
                    <div class="btn-login btn-login-page">
                        <button type="submit"  class="btn form-control btn-primary"><?= lang('Label.lbl_login') ?></button>
                            
                    </div>

                    <div class="col-md-12 text-center" style="line-height: 0px;">
                        <span class="note-p note-p-dn "><a href="/quen-mat-khau"
                                 class="forgot-Password"><?= lang('Label.lbl_forgotPassword'); ?></a></span>

                        <div class="wrap-dn-now wrap-dn-now-dn">
                            <fb:login-button scope="public_profile,email" onlogin="checkLoginState();"
                                id="loginFacebook" style="line-height: 25px;">
                            </fb:login-button>

                            <a href="<?php echo $loginGoogle; ?>"> <img
                                    src="<?php echo base_url('/public/images/Gmail.svg') ?>" alt="dang-nhap-gmail"
                                    class="img-fluid"></a>
                        </div>
                        <span style="font-size: 14px;"><?= lang('Label.lbl_noAccount') ?>?<a href="/dang-ky.html"
                            class="forgot-Password"> <?= lang('Label.lbl_register') ?></a></span>
                    </div>
                </form>
            </div>
        </div>
                            ...
                            <form action="<?= base_url('/vi/loginSocial'); ?>" id="form-add-phone" method="POST"
                                ...
                                <div class=" form-add-phone-ext">
                                    <input class="form-control frm-lg" id="addPhoneInput"
                                        oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');"
                                        name="phone" autocomplete="off" placeholder="<?= lang('Label.ph_phone') ?>"
                                        type="text">
                                    ...
                                    <input class="form-control frm-lg" id="accessToken" name="accessToken"
                                        autocomplete="off" value=" <?= $dataGoogle->ggToken ?>" placeholder=""
                                        type="hidden">
                                    <input class="form-control frm-lg" id="socialType" name="typeSocial"
                                        autocomplete="off" value="2" placeholder="" type="hidden">
                                    <?php } ?>
                                </div>
                                <div class=" btn-login">
